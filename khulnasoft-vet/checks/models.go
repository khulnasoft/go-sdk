...
var (
	modelsImpBlockList = []string{
		"github.com/khulnasoft/khulnasoft/modules/git",
	}
)

func checkModels(pass *analysis.Pass) (interface{}, error) {
	if !strings.EqualFold(pass.Pkg.Path(), "github.com/khulnasoft/khulnasoft/models") {
		return nil, nil
	}
	...
	for _, imp := range imps {
		if stringInSlice(imp, modelsImpBlockList) {
			pass.Reportf(0, "github.com/khulnasoft/khulnasoft/models cannot import the following packages: %s", modelsImpBlockList)
			return nil, nil
		}
	}
