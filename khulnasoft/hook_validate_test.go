...
// Hashers are based on https://github.com/go-khulnasoft/khulnasoft/blob/0dfc2e55ea258d2b1a3cd86e2b6f27a481e495ff/services/webhook/deliver.go#L105-L116
...
func TestVerifyWebhookSignatureHandler(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if tc.Signature != "" {
				req.Header.Set("X-Khulnasoft-Signature", tc.Signature)
			}
