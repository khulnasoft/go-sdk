...
func (c *Client) SearchRepos(opt SearchRepoOptions) ([]*Repository, *Response, error) {
	...
	if len(opt.RawQuery) != 0 {
		...
		// IsPrivate only works on khulnasoft >= 1.12.0
		if err := c.checkServerVersionGreaterThanOrEqual(version1_12_0); err != nil && opt.IsPrivate != nil {
			if *opt.IsPrivate {
				// private repos only not supported on khulnasoft <= 1.11.x
				return nil, nil, err
			}
			newQuery := link.Query()
...
// ArchiveType represent supported archive formats by khulnasoft
