...
const (
	...
	// GitServiceKhulnasoft represents a khulnasoft service
	GitServiceKhulnasoft GitServiceType = "khulnasoft"
...
func (opt *MigrateRepoOption) Validate(c *Client) error {
	...
	case GitServiceGitlab, GitServiceKhulnasoft:
		if len(opt.AuthToken) == 0 {
		...
		// https://github.com/go-khulnasoft/khulnasoft/pull/12672
		if c.checkServerVersionGreaterThanOrEqual(version1_13_0) != nil {
			return fmt.Errorf("migrate from service %s need khulnasoft >= 1.13.0", opt.Service)
		}
	case GitServiceGogs:
		if len(opt.AuthToken) == 0 {
		...
		if c.checkServerVersionGreaterThanOrEqual(version1_14_0) != nil {
			return fmt.Errorf("migrate from service gogs need khulnasoft >= 1.14.0")
		}
	}
	return nil
}
...
func (c *Client) MigrateRepo(opt MigrateRepoOption) (*Repository, *Response, error) {
	...
	if err := c.checkServerVersionGreaterThanOrEqual(version1_13_0); err != nil {
		if len(opt.AuthToken) != 0 {
			// khulnasoft <= 1.12 dont understand AuthToken
			opt.AuthUsername = opt.AuthToken
			opt.AuthPassword, opt.AuthToken = "", ""
		}
		if len(opt.RepoOwner) != 0 {
			// khulnasoft <= 1.12 dont understand RepoOwner
			...
			// khulnasoft <= 1.12 require RepoOwnerID
