...
// Client represents a thread-safe Khulnasoft API client.
type Client struct {
	url            string
	accessToken    string
	username       string
	...
	ignoreVersion  bool // only set by SetKhulnasoftVersion so don't need a mutex lock
}

// Response represents the khulnasoft response
...
// Usage of all khulnasoft.Client methods is concurrency-safe.
...
func (c *Client) doRequest(method, path string, header http.Header, body io.Reader) (*Response, error) {
	...
	if len(c.otp) != 0 {
		req.Header.Set("X-KHULNASOFT-OTP", c.otp)
	}
	if len(c.username) != 0 {
