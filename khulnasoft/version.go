...
func (c *Client) CheckServerVersionConstraint(constraint string) error {
	...
	if !check.Check(c.serverVersion) {
		...
		url := c.url
		c.mutex.RUnlock()
		return fmt.Errorf("khulnasoft server at %s does not satisfy version constraint %s", url, constraint)
	}
	return nil
}

// SetKhulnasoftVersion configures the Client to assume the given version of the
// Khulnasoft server, instead of querying the server for it when initializing.
// Use "" to skip all canonical ways in the SDK to check for versions
func SetKhulnasoftVersion(v string) ClientOption {
	if v == "" {
...
func (c *Client) checkServerVersionGreaterThanOrEqual(v *version.Version) error {
	...
	if !c.serverVersion.GreaterThanOrEqual(v) {
		...
		url := c.url
		c.mutex.RUnlock()
		return fmt.Errorf("khulnasoft server at %s is older than %s", url, v.Original())
	}
	return nil
}
