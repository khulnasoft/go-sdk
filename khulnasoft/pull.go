...
func (opt EditPullRequestOption) Validate(c *Client) error {
	...
	if len(opt.Base) != 0 {
		if err := c.checkServerVersionGreaterThanOrEqual(version1_12_0); err != nil {
			return fmt.Errorf("can not change base khulnasoft to old")
		}
	}
	return nil
}
...
// GetPullRequestDiff gets the diff of a PR. For Khulnasoft >= 1.16, you must set includeBinary to get an applicable diff
...
// fixPullHeadSha is a workaround for https://github.com/go-khulnasoft/khulnasoft/issues/12675
...
// which stays available to resolve the commit sha. This is fixed for khulnasoft >= 1.14.0
