...
func TestMyUser(t *testing.T) {
	...
	assert.EqualValues(t, "test01@khulnasoft.io", user.Email)
	assert.EqualValues(t, "", user.FullName)
	assert.EqualValues(t, "http://khulnasoft:3000/avatars/d794373e882a68fb173cef817fb6180a", user.AvatarURL)
...
func TestUserApp(t *testing.T) {
	...
	assert.EqualValues(t, "khulnasoft-admin", result[0].Name)
...
func TestUserEmail(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Len(t, el, 1)
	assert.EqualValues(t, "TestUserEmail@khulnasoft.io", el[0].Email)
...
func createTestUser(t *testing.T, username string, client *Client) *User {
	...
	user, _, err := client.AdminCreateUser(CreateUserOption{Username: username, Password: username + "!1234", Email: username + "@khulnasoft.io", MustChangePassword: OptionalBool(false), SendNotify: false})
	assert.NoError(t, err)
	return user
}
